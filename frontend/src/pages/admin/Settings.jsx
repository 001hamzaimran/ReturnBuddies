        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                ...
                <input
                    ...
                    value={val}
                    onChange={(e) =>
                        setVal(parseInt(e.target.value || "0", 10))
                    }
                ...
                <div className="mt-6 flex justify-end gap-2">
                    ...
                    <button
                        onClick={() =>
                            onSave(Math.max(0, Number(val) || 0))
                        }
...
export default function PickupSlotsDashboard() {
    ...
    async function handleDisableDay() {
        ...
        if ((dayData.dayBooked || totalBooked) > 0) {
            setConfirmModal({
                open: true,
                message: `This day already has ${(dayData.dayBooked || totalBooked)} bookings. 
                      Existing pickups stay, no new bookings allowed. Continue?`,
                onConfirm: async () => {
                    try {
    ...
    return (
        <div className="mx-auto max-w-6xl p-6">
            ...
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold text-gray-900">Pickup Slots Dashboard</h1>
                <button
                    ...
                    {dayData && (
                        <div className="mb-6 rounded-2xl border bg-white p-4 shadow-sm">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                <button onClick={handleDisableDay} className="rounded-xl bg-red-600 px-4 py-2 text-white hover:bg-red-700">
                                    Disable Entire Day
                                </button>
                                <button onClick={handleEnableDay} className="rounded-xl bg-green-600 px-4 py-2 text-white hover:bg-green-700">
                                    Enable Entire Day
                                </button>
                                <button
                                    onClick={() =>
                                        setCapacityModal({ open: true, slot: { timeSlot: null, capacity: dayData.dayCapacity || 0 } })
                                    }
            ...
            {viewMode === "weekly" && (
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <button onClick={() => setWeekStart(subWeeks(weekStart, 1))} className="rounded px-3 py-1 border">← Previous</button>
                        <div className="font-semibold">
                            Week of {format(weekStart, "yyyy-MM-dd")}
                        </div>
                        <button onClick={() => setWeekStart(addWeeks(weekStart, 1))} className="rounded px-3 py-1 border">Next →</button>
                    </div>
