...
export default function PromoCode() {
    ...
    const handleAddPromo = async () => {
        ...
        if (!promoCode || !discount || (activeTab === "single" && !selectedUserId)) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">Promo Codes</h1>
                    ...
                    <tbody>
                        {currentPromos.map((promo) => (
                            <tr key={promo._id} className="border-t hover:bg-gray-100">
                                <td className="p-3 sm:p-5 font-medium text-gray-800">{promo.PromoCode}</td>
                                <td className="p-3 sm:p-5">{promo.Discount}%</td>
                                <td className="p-3 sm:p-5">{promo.userId}</td>
                                <td className="p-3 sm:p-5">{new Date(promo.createdAt).toLocaleString()}</td>
                                <td className="p-3 sm:p-5 flex flex-wrap gap-2">
                                    <button className="bg-green-600 text-white px-2 py-1 rounded text-xs sm:text-sm">Edit</button>
                                    <button
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-2">
                    <div className="bg-white rounded-xl shadow-lg w-full max-w-screen-sm sm:max-w-lg p-4 sm:p-6 relative">
                        ...
                        <h2 className="text-lg sm:text-xl font-bold text-gray-800 mb-4">Add Promo Code</h2>

                        {/* Tabs */}
                        <div className="flex border-b mb-4 text-sm">
                            <button
                                onClick={() => setActiveTab("all")}
                                className={`px-3 py-2 sm:px-4 font-medium ${activeTab === "all"
                            ...
                            <button
                                ...
                                className={`ml-2 sm:ml-4 px-3 py-2 sm:px-4 font-medium ${activeTab === "single"
                                    ? "border-b-2 border-purple-600 text-purple-600"
                                    : "text-gray-500 hover:text-purple-600"
                                    }`}
                            >
                        ...
                        >
                            {activeTab === "all" ? "Add Promo for All Users" : "Add Promo for Single User"}
                        </button>
                    </div>
                </div>
            )}
