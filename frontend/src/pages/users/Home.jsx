...
export default function Home() {
  ...
  const GetItems = async () => {
    const res = await axios.get(
      "https://returnbuddies-production.up.railway.app/api/getitems"
    );
    const data = res.data;
    setItems(data.items);
      ...
      {items && items.length > 0 ? (
        ...
        <div className=" flex flex-col items-center justify-center bg-white text-center px-4">
          ...
          <p className="text-gray-500 max-w-md mt-2">
            Add orders manually or scan tracking numbers or QR Codes to schedule
            a pickup. Packaging is optional.
          </p>

          {/* Floating Plus Button */}
          <div className="fixed bottom-6 w-full flex justify-center">
            <button
              className="bg-[#d491ff] text-white p-4 rounded-full text-2xl shadow-lg cursor-pointer hover:bg-[#c07dfd]"
              onClick={handleNavigate}
            >
