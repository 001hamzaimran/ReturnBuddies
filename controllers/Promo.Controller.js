...
const getPromo = async (req, res) => {
    try {
        ...
        if (!userid) {
            return res.status(400).json({ message: "User ID is required in headers", success: false });
        }

        if (!discountCode) {
            return res.status(400).json({ message: "Promo code is required in query or param", success: false });
        }
        ...
        if (!promo) {
            return res.status(404).json({ message: "Promo code not found or not available for this user", success: false });
        }

        return res.status(200).json({ message: "Promo code found", success: true, promo });
    } catch (error) {
