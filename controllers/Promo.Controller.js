...
const getPromo = async (req, res) => {
    try {
        ...
        if (!promo) {
            return res.status(200).json({ status: 201, message: "Promo code not found or not available for this user", success: false });
        }

        return res.status(200).json({ status: 200, message: `You get ${promo.Discount}% discount`, success: true, promo });
    } catch (error) {
