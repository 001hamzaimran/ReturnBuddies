...
function getSlotStatus(slot) {
  ...
  if (
    slot.capacity > 0 &&
    slot.pickupsBooked >= Math.floor(0.8 * slot.capacity)
  )
    return "almost-full";
  return "available";
}
...
export const disableSlot = async (req, res) => {
  try {
    ...
    const existing = await DisabledSlot.findOne({
      date,
    ...
    res
      .status(200)
      .json({
        message: "Pickup slot disabled successfully.",
        data: newDisabledSlot,
        status: 200,
      });
  } catch (error) {
...
export const getDisabledSlots = async (req, res) => {
  try {
    ...
    const groupedData = dates.map((d) => {
      ...
      const daySlots = Object.entries(TIME_SLOTS).map(
        ([timeSlot, capacity]) => {
  ...
  try {
    ...
    if (value === true) {
      ...
      if (!existing) {
        ...
        return res
          .status(201)
          .json({ message: "Slot disabled successfully.", data: newSlot });
      }
...
export const getSlotData = async (req, res) => {
  try {
    ...
    const dayDisabled = disabledSlots.some(
      (s) => s.timeSlot === null && s.disabled
    );
    ...
    const slots = defaultTimeSlots.map((timeSlot) => {
      ...
      return {
        ...
        capacity: slotRecord
          ? slotRecord.capacity
          : timeSlot === "9:00 AM to 6:00 PM"
          ? 10
          : timeSlot === "9:00 AM to 1:00 PM"
          ? 8
          : timeSlot === "11:00 AM to 3:00 PM"
          ? 5
          : timeSlot === "2:00 PM to 6:00 PM"
          ? 4
          : 5,
        pickupsBooked,
      };
...
export const getPickupsForSlot = async (req, res) => {
  try {
    ...
    const pickups = await pickupModel.find(
      ...
      "_id PickupName status phone pickupDate pickupTime"
...
export const setSlotCapacity = async (req, res) => {
  try {
    ...
    if (!date || !timeSlot || !capacity) {
      return res
        .status(400)
        .json({ error: "Date, timeSlot and capacity are required." });
    }
