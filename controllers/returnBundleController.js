...
export const createProductItemsAndReturnBundle = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(200)
        .json({ message: "Missing userId in params or headers.", status: 400 });
    }

    if (!rawItems) {
      return res
        .status(200)
        .json({ message: "Missing items in request body.", status: 400 });
    }
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Invalid JSON in items.", status: 400 });
    }

    if (!Array.isArray(items) || items.length !== files.length) {
      return res
        .status(200)
        .json({ message: "Mismatch between items and files.", status: 400 });
    }
    ...
    const populatedBundle = await ReturnBundle.findById(
      savedBundle._id
    ).populate("products");
...
export const getReturnBundle = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(200)
        .json({ error: "User ID is required", status: 400, success: false });
    }
...
export const getAllReturnBundles = async (req, res) => {
  try {
    ...
    const bundles = await ReturnBundle.find({
      userId,
      _id: {
        $nin: excludedBundleIds.map((id) => new mongoose.Types.ObjectId(id)),
      },
    }).populate("products");
...
export const DeleteBundle = async (req, res) => {
  try {
    ...
    if (!bundleId) {
      return res
        .status(200)
        .json({ error: "Bundle ID is required", status: 400, success: false });
    }
    ...
    if (!bundle) {
      return res
        .status(200)
        .json({ error: "Bundle not found", status: 404, success: false });
    }
